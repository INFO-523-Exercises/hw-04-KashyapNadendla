...
In a simple linear regression,the best fit model indicates that there is a decrease in prey per month with increase in age.
...
# Train/test split
numTrain <- 86  # number of training instances
...
X4 <- 0.5 * cats_uk_reference$prey_p_month + rnorm(numInstances, mean=0, sd=0.01)
...
plot2 <- ggplot() +
  geom_point(aes(X2, X3), color='purple1') +
  xlab('X2') + ylab('X3') +
...
length_X1 <- length(X_train2[, 1])
length_X2 <- length(X_train2[, 2])
length_y <- length(y_train)

# Print the lengths
cat("Length of X1:", length_X1, "\n")
cat("Length of X2:", length_X2, "\n")
cat("Length of y:", length_y, "\n")
```
